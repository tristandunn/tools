...
get '/' do
  ...
  @players = Player.all.map do |player|
    ...
    best_finish = player.best_finish_last_year
    ...
    function sortTable(column) {
      ...
      rows.sort((a, b) => {
        ...
        switch(column) {
          ...
          case 'best-finish':
            // Lower place numbers are better (1st place is better than 2nd)
            aVal = parseInt(a.dataset.bestFinish) || 999;
            bVal = parseInt(b.dataset.bestFinish) || 999;
            // Reverse the comparison so lower numbers rank higher
            return currentSort.direction === 'asc'
              ? bVal - aVal
              : aVal - bVal;
  ...
  <table class="min-w-full divide-y divide-gray-200">
    <thead class="bg-gray-50">
      <tr>
        ...
        <th scope="col" class="sortable cursor-pointer px-2 sm:px-4 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hover:text-gray-700" data-sort="best-finish">
          Best Finish <span class="sort-indicator"></span>
        </th>
      </tr>
    </thead>
    <tbody class="bg-white divide-y divide-gray-200">
      <% @players.each_with_index do |stats, index| %>
        <tr class="hover:bg-gray-50" data-total="<%= stats[:total] %>" data-average="<%= stats[:average] || -999 %>" data-name="<%= stats[:player].name.downcase %>" data-nickname="<%= (stats[:player].nickname || '').downcase %>" data-latest-year="<%= stats[:latest_year] %>" data-elo="<%= stats[:elo] %>" data-win-pct="<%= stats[:win_pct] %>" data-best-finish="<%= stats[:best_finish] || 999 %>">
          ...
          <td class="px-2 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
            <% if stats[:best_finish] %>
              <span class="font-semibold text-yellow-600"><%= stats[:best_finish] %></span>
            <% else %>
