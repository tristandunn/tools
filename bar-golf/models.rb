...
class Player < ActiveRecord::Base
  ...
  has_many :tournaments, through: :tournament_results
  ...
  # Get best tournament finish in the last year
  # Returns the lowest place number (1st place is better than 2nd)
  def best_finish_last_year
    one_year_ago = Time.now.year - 1

    best_result = tournament_results
      .joins(:tournament)
      .where("tournaments.year >= ?", one_year_ago)
      .order(:place)
      .first
...
class Tournament < ActiveRecord::Base
  has_many :tournament_results, dependent: :destroy
  has_many :players, through: :tournament_results
...
def setup_schema
  ActiveRecord::Schema.define do
    ...
    unless ActiveRecord::Base.connection.table_exists?(:tournament_results)
